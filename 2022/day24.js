...
function findGoal(oState, step = 0) {
  ...
  ;[
    ...
    // Find valid moves
    .filter((tile) => {
      return tile != null && !tile.wall && tile.blizzards.length == 0
...
function forEachTile(state, cb) {
  for (let y = 0; y <= state.dimensions.max.y; y++)
    for (let x = 0; x <= state.dimensions.max.x; x++) cb(state.map[y][x])
}
...
function printState(state, minute) {
  ...
  forEachTile(state, (p, x, y) => {
    ...
    if (p.x == state.dimensions.max.x) msg += "\n\t"
  })
  console.log(msg)
  console.log(
    `  Map dimensions: ${state.dimensions.max.x} x ${state.dimensions.max.y}`
  )
  console.log(`  Current pos: ${state.pos.x},${state.pos.y}`)
  console.log(`  Goal:        ${state.goal.x},${state.goal.y}`)
  if (minute) console.log("Minute: ", minute)
}
