...
const puzzle = new Puzzle("Day 12, 2022")
...
const parseInput = (input) => {
  ...
  for (let y = 0; y < rows.length; y++) {
    ...
    for (let x = 0; x < arr.length; x++) {
      ...
      if (cell.height == 1) starts.push(cell)
...
const printPath = (path) => {
  let msg = "\npath:\n"
  let cds = path.map((p) => {
    return { x: p[0].split(",")[0], y: p[0].split(",")[1], m: p[1] }
  })
  //console.log(cds);
  ...
  for (let col in map) {
    for (let row in map[col]) {
      ...
      if (start.x == row && start.y == col) msg += "O"
        ...
        col == 0 ||
        col == Object.keys(map).length - 1 ||
        row == 0 ||
        row == Object.keys(map[0]).length - 1
      )
        msg += " "
      else msg += " "
    }
...
puzzle.setPart2((rawinput) => {
  ...
  //printPath(bestPath)
  return p2paths.sort((a, b) => a.length - b.length)[0].length
})
