...
const expectedPt1 = 6440
const expectedPt2 = 5905

const rawInput = `55559 2
Q525J 656
457J4 324
T6668 259
38847 640
...
const pt2 = process.argv[2]?.match("2") || true
...
const parseInput = (input, pt2 = false) => {
  return input.split("\n").map((row) => {
    const hand = row.split(" ")[0]
    return {
      hand,
      bet: Number(row.split(" ")[1]),
      score: determineHand(hand, pt2),
    }
  })
}

const determineHand = (hand, pt2 = false) => {
  ...
  const result = Object.entries(sets)
    .sort((a, b) => {
      ...
      // Then card rank
      return cardRanks.indexOf(b[0]) - cardRanks.indexOf(a[0])
    })
    .reduce(
      (res, [card, count]) => {
        ...
        if (res == null && count + jokers == 5)
          ...
          return {
            type: "2pair",
            card:
              cardRanks.indexOf(res.card) < cardRanks.indexOf(card)
                ? res.card + card
                : card + res.card,
          }
  ...
  //console.log(result.type, result.card, "\n")
  return result
}
  ...
  return hands.sort((h1, h2) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (h1.hand[i] != h2.hand[i]) {
        return cardRanks.indexOf(h2.hand[i]) - cardRanks.indexOf(h1.hand[i])
      }
    }
    return 0
  })
...

{
  ...
  const sortedHands = sortHands(input)

  let output = addUpScores(sortedHands)

  testing && console.log("Expected output:", expectedPt1)
  console.log("Output:", output)
}

// Update card ranks for part 2
...

{
  if (pt2) {
    ...
    const input = parseInput(testing ? testInput1 : rawInput, true)

    const hands = sortHands(input)
